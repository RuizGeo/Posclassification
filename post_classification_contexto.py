...
def GetLabelsNeighbors(regioes_props, classificacao,nodata):
    ...
    for i, regprop in enumerate(regioes_props[10:]):   
        if regprop.area < 1000:
            classes=np.array([0,1,2,3,4,5,6,7])
            #avaliar areas menores que o limiar
            #if reg[regprop.coords[0,0],regprop.coords[0,1]] < 0.85:
            print'###################################'
            total+=1
            ...
            if np.unique(classNearest).size ==0:
                pass
            else:
                #obter classes
...
#criar amostras de
f_seg= '/home/ruiz/Documentos/Pesquisas/OBIA_KNN/Segmentacao/seg_corte.tif'
...
classificacaoPos,dtype=GetLabelsNeighbors(regioes_props, classificacao, nodatavalue)